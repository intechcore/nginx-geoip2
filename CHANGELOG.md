...
- `SECURITY.md`, `CONTRIBUTING.md` and `.editorconfig` follow the shared templates. The changelog
  keeps one Unreleased section; the entries released by hand moved below.
...
- A release fails when the package API cannot list the published tags, instead of counting from
  an empty list. It no longer asks for the deleted `nginx-geoip` package; the git tags keep its
  build numbers.

## History before automatic releases

These entries were released by hand, up to 1.31.6-1 (2026-09-16). The first image,
1.29.5-1, predates this changelog; see the git log for it.

### 1.30.0-2 to 1.31.6-1
